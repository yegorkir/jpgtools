...
## Веб-приложение (GitHub Pages)

В `docs/` лежит браузерная версия компрессии JPEG (только `compress`).
Она работает полностью локально в браузере и не отправляет файлы на
сервер. Для публикации в GitHub Pages:

1. В настройках репозитория откройте **Settings → Pages**.
2. В качестве источника выберите ветку `main` и папку `/docs`.
3. Сохраните — сайт будет доступен по ссылке GitHub Pages.

Локально можно открыть `docs/index.html` двойным кликом.
